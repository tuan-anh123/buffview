...
class Main:
    def __init__(self):
        ...
        cursor.hide()
        txt=self.format(" ĐÀO PHẠM TUẤN ANH ", " ID Video Tik Tok :  ")
        YourId=input(txt)
        self.videos = YourId
...
░░░╚═╝░░░░╚═════╝░╚═╝░░╚═╝╚═╝░░╚══╝  ╚═╝░░╚═╝╚═╝░░╚══╝╚═╝░░╚═╝           \n                    \n\n\n\n\n"""
    ...
    def solve_captcha(self, sessid):
        try:
            ...
            print(self.format("ĐÀO PHẠM TUẤN ANH ",f" Solved captcha ! | {captcha_answer}"))
            return alpha_key
            ...
            print(
                self.format(
                    "!",
                    "Captcha Invalid | Check access to Zefoy | Facebook or Youtube : ĐÀO PHẠM TUẤN ANH ",
                )
            )
            self.solve_captcha(sessid)
    def get_sessid(self):
        ...
        while True:
            try:
                ...
                if "This service is currently not working" in decryped_answer:
                    ...
                    print(self.format(" ĐÀO PHẠM TUẤN ANH ",f" Delay: {timer}     "), end="\r")
                    start = time.time()
                    while time.time() < start + timer:
                        print("\r", end="")
                        print(self.format("ĐÀO PHẠM TUẤN ANH ", f" Delay: {round((start + timer) - time.time())}       "),end="\r")
                        time.sleep(1)
                    continue
                ...
                if latency > 3:
                    print(self.format(" ĐÀO PHẠM TUẤN ANH ",f" Sent views ID: {aweme_id}"))
                ...
                print(self.format(" TUAN ANH ",f" Delay: {timer}    "), end="\r")
                start = time.time()
                while time.time() < start + timer:
                    print("\r", end="")
                    print(self.format(" DUMA TỪ TỪ CHỜ  ",f" Delay: {round((start + timer) - time.time())}     "), end="\r")
                    time.sleep(1)
            except:
                os.system("python " + sys.argv[0])
                sys.exit(0)
...
class Menu:
    def format(self, symbol, text):
        ...
        return f"""   {Col.Symbol(symbol, self.lblue, self.blue)} {self.lblue}{text}{Col.reset}"""
    def main(self):
        banner=requests.get("https://pastebin.com/raw/rFbWH9q2").text
